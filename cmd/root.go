...
func init() {
	...
	// subscriptionID := h.GetEnvVarOrExit("AZURE_SUBSCRIPTION_ID")
	// authorizer, err := utils.GetAuthorizer(azure.PublicCloud)
	// h.OnErrorFail(err, "utils.GetAuthorizer failed")
	...
	rootCmd.PersistentFlags().StringP("location", "l", "canadaeast", "Default location to manage resources on Azure")
...
func initConfig() {
	...
	if cfgFile != "" {
		...
		// Search config in home directory with name ".oh" (without extension).
